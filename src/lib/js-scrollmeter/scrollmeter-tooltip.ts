...
export class ScrollmeterTooltip extends IScrollmeter {
  ...
  public createTimelineTooltip = (
    ...
    if (this.#scrollmeter.getDefaultOptions().usePreview) {
      const dataUrl = this.#cropImageAtPercent(targetElement.offsetTop)
    ...
    timelineTooltip.classList.add(
      styles[`scrollmeter_timeline_tooltip_${direction}`],
    )
    ...
    if (defaultOptions && defaultOptions.tooltipOptions) {
      ...
      if (background) {
        this.#scrollmeter
          ...
          ?.style.setProperty('--scrollmeter-tooltip-background', background)
      }
      if (fontColor) {
        this.#scrollmeter
          .getScrollmeterContainer()
          ?.style.setProperty('--scrollmeter-tooltip-font-color', fontColor)
      }
      if (fontSize) {
        this.#scrollmeter
          .getScrollmeterContainer()
          ?.style.setProperty(
            '--scrollmeter-tooltip-font-size',
            `${fontSize}px`,
          )
      }
      if (paddingBlock) {
        this.#scrollmeter
          .getScrollmeterContainer()
          ?.style.setProperty(
            '--scrollmeter-tooltip-padding-block',
            `${paddingBlock}px`,
          )
      }
      if (paddingInline) {
        this.#scrollmeter
          .getScrollmeterContainer()
          ?.style.setProperty(
            '--scrollmeter-tooltip-padding-inline',
            `${paddingInline}px`,
          )
      }
      if (width) {
        this.#scrollmeter
          .getScrollmeterContainer()
          ?.style.setProperty('--scrollmeter-tooltip-width', `${width}px`)
      }
    }
  }
}
