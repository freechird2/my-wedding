  ...
  const handleShareKakao = () => {
    window.Kakao.Share.sendDefault({
      ...
      content: {
        ...
        description: `${format(parseISO(date), 'yyyy년 M월 d일 eeee aaa h시', { locale: ko })}`,
        imageUrl:
          'https://www.google.com/url?sa=i&url=https%3A%2F%2Fnews.mt.co.kr%2Fmtview.php%3Fno%3D2024090810104568867&psig=AOvVaw0CdbXhss-i72QYDyn1TxKu&ust=1733364414420000&source=images&cd=vfe&opi=89978449&ved=0CBQQjRxqFwoTCNCDqOODjYoDFQAAAAAdAAAAABAE',
        link: {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (!window.Kakao.isInitialized()) {
        window.Kakao.init(process.env.REACT_APP_KAKAO_APP_KEY as string)
      }
    }
  }, [])
...
const IconKakao = ({ className }: { className: string }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 32 32"
